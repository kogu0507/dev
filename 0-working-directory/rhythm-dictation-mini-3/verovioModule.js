...
export async function initializeVerovio(meiTemplate) {
    globalMeiTemplate = meiTemplate; // テンプレートをモジュール内に保持 (ここではMEIテンプレートの初期設定にのみ利用)
 ...
 * @param {string} meiTemplate - MEI XMLのテンプレート文字列。（追加された引数）
 * @returns {string} 完全なMEI XML文字列。
 */
export function generateMeiFromRhythm(rhythm1, rhythm2, tempo, meiTemplate) { // ★ ここに meiTemplate 引数を追加 ★
    // globalMeiTemplate の代わりに引数で渡された meiTemplate を使う
    if (!meiTemplate) { // ★ ここも引数で渡された meiTemplate をチェック ★
        console.error('MEIテンプレートが提供されていません。');
        return '';
    }
    if (!rhythm1 || !rhythm2) {
...
/**
 * MEI の文字列、または事前にパースされた Midi オブジェクトを受け取り、Tone.jsで再生する。
 * @param {string|Object} meiOrMidi - 再生したいリズムのMEI XML文字列、または @tonejs/midi の Midi オブジェクト。
 ...
 * @param {boolean} [forceMeiParsing=false] - meiOrMidi が MEI 文字列の場合でも、常にMEIからMIDIを生成し直すか。
 * (ユーザー選択音源のように、毎回MIDIを生成したい場合にtrue)
 */
export async function playRhythmMidi(meiOrMidi, Tone, Midi, synth, volume, forceMeiParsing = false) {
    if (!verovioManagerInstance || !Tone || !Midi || !synth) {
        console.error('必要なモジュール（VerovioManager, Tone.js, @tonejs/midi, シンセサイザー）がまだ初期化されていません。');
        return;
    }
    if (!meiOrMidi) {
        console.warn('再生するデータがありません。');
        return;
    }

    let midi;

    // meiOrMidi が文字列（MEI）で、かつ forceMeiParsing が true の場合、または Midi オブジェクトではない場合
    if (typeof meiOrMidi === 'string' || forceMeiParsing) {
        if (typeof meiOrMidi !== 'string') {
            console.warn('playRhythmMidi: forceMeiParsingがtrueですが、meiOrMidiが文字列ではありません。');
            return;
        }
        try {
            const meiDataUri = 'data:application/mei+xml;charset=utf-8,' + encodeURIComponent(meiOrMidi);
            const midiBuffer = await verovioManagerInstance.getMidiFromMei(meiDataUri);
            midi = new Midi(midiBuffer);
        } catch (err) {
            console.error('MEIからMIDIの生成に失敗しました:', err);
            return;
        }
    } else if (meiOrMidi instanceof Midi) { // meiOrMidi が Midi オブジェクトの場合
        midi = meiOrMidi;
    } else {
        console.error('playRhythmMidi: 不正な meiOrMidi 引数です。MEI文字列またはMidiオブジェクトが必要です。');
        return;
    }
